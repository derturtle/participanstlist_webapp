...
function strCreateCtrlButtons($url, $position) {
    $but_back = '<a href="' . $url . '" class="btn btn-secondary btn-back mr-2" onclick="goBack()"><i class="fa-solid fa-arrow-left-long"></i></a>';
...
function strCreateEditEntry($position) {
    ...
    $ret_str .= '<input type="text" id="year-of-birth-' . $position . '" placeholder="Year of Birth">';
...
function strCtreateTableHead($data) {
    ...
    $ret_str .= '<th class="fixed-column">Name</th>';
    ...
    for ($i = 4; $i < count($data[0]); $i++) {
        ...
        // Create entry
        $ret_str .= '<th ' . $hstyle . '>' . $dayOfWeek . ' ' . $date . '</th>';
    }
...
function strGenerateCheckboxes($data, $actIndex , $today) {
    ...
    for ($j = 4; $j < count($data[0]); $j++) {
        ...
        if ($isToday) {
            ...
            $ret_str .= '<input type="checkbox" id="checkbox-' . $i . '-' . $j . '" ' . $isChecked . ' onclick="toggleCheckbox(this)">';
        } else {
            // no today disbale checkbox
            $ret_str .= '<input type="checkbox" id="checkbox-' . $i . '-' . $j . '" ' . $isChecked . ' onclick="return false;" class="greyed-out-checkbox">';
        }
        $ret_str .= '</td>';
    }
    return $ret_str;
